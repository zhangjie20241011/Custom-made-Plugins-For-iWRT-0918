...
return view.extend({
	...
	render: function(value) {
		var header = [
			E('h2', {'class': 'section-title'}, _('DNS泄漏测试')),
			E('div', {'class': 'cbi-map-descr'}, _('执行 DNS 泄漏测试以检查您的 DNS 设置的安全性。'))
		];
		var result;
		if (value.length > 0) {
			...
			result = [
				E('table', {'class': 'table cbi-section-table'}, [
					E('tr', {'class': 'tr table-titles'}, [
						...
						E('th', {'class': 'th'}, _('国家')),
						E('th', {'class': 'th'}, _('DNS 服务器')),
						E('th', {'class': 'th'}, _('已找到的服务器')),
						E('th', {'class': 'th'}, _('检测结果')),
						E('th', {'class': 'th'})
					]),
					E(rows)
				])
		...
		return E('div', {'class': 'cbi-map'}, [
			...
			E('div', {'class': 'cbi-section'}, [
				E('h3', {'class': 'section-title'}, _('检测结果')),
				E(result)
			]),
		])
	}
